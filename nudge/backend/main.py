...
from services.market_service import fetch_stock_data, get_market_status, resolve_ticker
...
def execute_sell(req: ExecuteSellRequest, db: Session = Depends(get_db)):
    ...
    # Fetch current price
    data = fetch_stock_data(req.ticker, period='5d', interval='1d')
    price = float(data.iloc[-1].item()) if len(data) else holding.avg_buy_price
...
def get_portfolio(user_id: str, db: Session = Depends(get_db)):
    ...
    for h in holdings:
        try:
            data = fetch_stock_data(h.ticker, period='5d', interval='1d')
            if len(data) == 0:
                raise ValueError("No valid data")
            current = float(data.iloc[-1].item())
            prev    = float(data.iloc[-2].item()) if len(data) > 1 else current
...
def get_timeline(user_id: str, db: Session = Depends(get_db)):
    ...
    return get_timeline_events(user_id, db)
...
def portfolio_health(user_id: str, db: Session = Depends(get_db)):
    ...
    return compute_portfolio_health(user_id, db)
...
def insight_cards(user_id: str, db: Session = Depends(get_db)):
    ...
    return generate_insight_cards(user_id, db)
...
def behavioral_profile(user_id: str, db: Session = Depends(get_db)):
    ...
    return compute_behavioral_profile(user_id, db)
